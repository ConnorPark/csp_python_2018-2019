...
def get_images(directory=None):
    ...
    logo = os.path.join(directory, 'unicornlogo.png')
    ...
    for entry in directory_list:
        ...
        try:
            image.paste(unicornlogo.png, (1162, 966), mask=earth_small)
            file_list += [entry]
            image_list += [image]
        except IOError:
...
def round_corners_of_all_images(directory=None):
    ves a modfied version of each image in directory.
    ...
    for n in range(len(image_list)):
        ...
        curr_image.paste(logo, (1162, 966), mask=logo)
        # Save the altered image, suing PNG to retain transparency
        curr_image_filename = os.path.join(new_directory, filename + '.png')
        curr_image.save(curr_image_filename)
