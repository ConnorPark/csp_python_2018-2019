...
def insaniTea_border(width, height, original_image):
    ...
    #concerts image to array for pixel editing
    image = np.array(img)
    for row in range(width):
        for column in range(height):
            if (row+column)/5 % 2 == 0: 
                ...
                image[-row][column] = [255, 127, 127, 50] # slightly opaque area
                if width > 60:
                    image[-row][column] = [0, 127, 127, 220]#more opaque area
                    #causes cross hatching of lines
        
            else:
                # Odd stripe
                image[row][column] = [255, 0, 255, 0] # eliminates parts of 
                #image in area
    #creates opaque stripes
    for row in range(width):
        for column in range(height):
            column = row**2 - 2*row + 1
            if column in range(height):
                image[row][column] = [0, 0, 0, 225]
    ##abandoned code below meant to create a hollow space for image without 
    ##geometric pattern
    ...
    #creates space for unmasked image
    for row in range(width):
        for column in range(height):
            if row in range(width // 8, 7 * width // 8) and column in range(height // 8, 7 * height // 8):
                image[row][column] = [0, 0, 0, 255]
    
    #converts array back to useable pil mask
    mask_image = PIL.Image.fromarray(image)
    mask_image = mask_image.resize((width, height))
    
    mask_in_use = mask_image
    
    #returns mask to be used on image
    return mask_in_use
...
def border(width, height, original_image):
    ...
    #Clears entire area in mask
    for row in range(width):
        for column in range(height):
            image2[row][column] = [0, 0, 0, 0]
    
    #sets area of image to remain in final product
    for row in range(width):
        for column in range(height):
            if row in range(width // 8, 7 * width // 8) and column in range(height // 8, 7 * height // 8):
...
def shading_for_image(width, height, original_image):
    ...
    for row in range(width):
        for column in range(height):
            ...
            ##following abandoned code was designed to subtract alpha value from
            ##sumation of the pixel
            #if shading_image[row][column][3] != 255:
                #total_value = sum(shading_image[row][column]) - shading_image[row][column][3]
            #else:
                #total_value = 500
            
            #sums RGB pixel values
            total_value = shading_image[row][column][0] + shading_image[row][column][1] + shading_image[row][column][2]
            
            ##Abandoned code is developer check RGB summation did not exceed 665
            #print (total_value)
            
            ##Abandoned code was attempt to correct runtime warning
            #temp_shading = PIL.Image.fromarray(shading_image)
            #temp_shading.convert("RGBA")
            
            ##Abandoned code was used to create shading contors
    ...
    #use grayscale on image
    return shade
    
##Below is an abandoned function. It was meant to be used to combine images
...
def combine(width, height, original_image, curr_result):
    ...
    width3, height3 = curr_result.size
    
    #poster = PIL.Image.new("RGBA", (1000, 500))
    temp_poster = PIL.Image.new("RGBA", (width + width3, height))
    #draw_mask = PIL.Image.new("RGBA", (width, height))
    #mask = PIL.ImageDraw.Draw(draw_mask)
    
    #mask.polygon([(width//4 ,0),(3 * width // 4,0),
                            #(3 * width // 4, height),(width//4, height)],
                            #fill=(127,0,127,255))
    
    #temp_poster.paste(original_image, (0,0), mask=mask)
    #curr_result = curr_result.resize((width, height))
    
    #draw_mask= draw_mask.resize((int(1 * width), int(1 * height)))
    
    #temp_poster.paste(draw_mask, ((width // 2 ) - int(.5 * width), (height // 2 ) - int(.5 * height)))
    
    #temp_poster.paste(curr_result, (width//2, 0), mask=mask)
    
    temp_poster.paste(original_image, (0,0))
    temp_poster.paste(curr_result, (width3, 0))
    
    return temp_poster
    

def rounded_corners(width, height, percent_of_side=.3):
    '''
...
def modify_one_image(original_image, chosen_mask, code, percent_of_side=.3):
    ...
    picking_mask = raw_input("Which mask would you like? Rounded mask, \
Border mask, insaniTea or combine (to combine the original and result)? ")
    #set the radius of the rounded corners
    print("What color would you like the border to be?:")
    time.sleep(.5)
    print("You can type: red, green, blue, pruple, white, black or none.")
    time.sleep(.5)
    ...
    if picking_color == 'red':
        ...
        print('Do not enter a value less than 0 or greater than 255.')
        time.sleep(0.5)
        red = int(raw_input())
        green = int(raw_input())
        blue = int(raw_input())
    ...
    text = raw_input()
    wrap_text = textwrap.wrap(text, width=10)
    scalar = len(wrap_text)
    while scalar >= 5:
        print ("Please keep the text to less than 20 characters: ")
        text = raw_input()
        wrap_text = textwrap.wrap(text, width=10)
        scalar = len(wrap_text)
    else:
...
#-------------------------------------------------------------------------------
    ...
    '''if shading_yes_no.lower() == 'yes':
        ...
        #shaded_result = PIL.Image.new('RGBA', original_image.size, (0,0,0,0))
        #shaded_result.paste(original_image, (0,0))
    ...
    if picking_mask.lower() == "rounded mask":
        #curr_result = result
        result.paste(original_image, (0,0), mask=rounded_corners(width, height))
        current_mask_in_use = rounded_corners(width, height)
        #masking_check = 1
    ...
    if picking_mask.lower() == "border mask":
        ...
        current_mask_in_use = border(width, height, original_image)
        #masking_check = 1
    ...
    if picking_mask.lower() == "insanitea":
        ...
        current_mask_in_use = insaniTea_border(width, height, original_image)
        #masking_check = 1
    ...
    if picking_mask.lower() != "border mask" and picking_mask.lower() != "rounded mask" and picking_mask.lower() != "insanitea":
        result.paste(original_image, (0,0))
        current_mask_in_use = PIL.Image.new("RGBA", (width, height), (0,0,0, 255))
        #combine_mask_in_use = rounded_corners(width, height)
    
    if shading_yes_no.lower() == 'yes':
        #result.paste(original_image, (0,0), mask=mask_in_use)
        shaded_result = PIL.Image.new('RGBA', original_image.size, (0,0,0,0))
        shaded_result.paste(original_image, (0,0))
        shaded_result = PIL.Image.alpha_composite(shaded_result, shading_for_image(width, height, original_image))
        result.paste(shaded_result, (0,0), mask=current_mask_in_use)
    #if shading_yes_no == 'no' and masking_check == 0:
    #    result.paste(original_image, (0,0))
     #   result = PIL.Image.alpha_composite(result, shading_for_image(width, height, original_image))
    if shading_yes_no.lower() == 'even shading':
        ...
        #result.paste(shading, (0, 0), mask=shading)
        shaded_result = PIL.Image.new('RGBA', original_image.size, (0,0,0,0))
        shaded_result = PIL.Image.alpha_composite(result, shadow)
        result.paste(shaded_result, (0,0), mask=current_mask_in_use)
    #else:
    #    result.paste(original_image, (0,0), mask=mask_in_use)
    ...
    if picking_mask.lower() == "combine":
        ...
        poster = combine(width, height, original_image, result)
        result = poster
        width4, height4 = result.size
        final_poster = PIL.Image.new("RGBA", (width4, height4), colorchose)
        final_poster.paste(poster, (0,0), mask=rounded_corners(width4, height4))
        result = final_poster
    
    width4, height4 = result.size
    
    result.paste(black_layer, ((width4 // 2) - (wlogo - 5), (height4 // 2) - hlogo), mask=logo) #new change
    result.paste(logo, ((width4 // 2) - wlogo, (height4 // 2) - hlogo), mask=logo)
    qr_code_width = qr_code_for_image.size[0]
    result.paste(qr_code_for_image, ((width4 // 2) - (qr_code_width//2), (height4 // 2) + hlogo), mask=qr_code_for_image)
    ...
    if text_width[0] <= width:
        width4, height4 = result.size
        add_text.text(((width4 // 2) - (text_width[0] // 2), (height4 // 8)), text, font=font, fill=(0, 0, 0, 255))
    elif text_width[0] >= width:
        width4, height4 = result.size
        wrap_text = textwrap.wrap(text, width=10)  #https://docs.python.org/2/library/textwrap.html#textwrap.TextWrapper.wrap
        #scalar = len(wrap_text)
        for n in range(len(wrap_text)):
            ...
            #add_text.text(((width4 // 2) - wrap_text_width, (height4 // 8) + num), str(wrap_text[n]), font=font, fill=(0, 0, 0, 255))
            add_text.text((0 + (width4 // 8), (height4 // 8) + num ), str(wrap_text[n]), font=font, fill=(0,0,0,255))
...
def modify_all_images(directory=None):
    ...
    for n in range(len(image_list)):
        # Parse the filename
        print("Image "+str(n+1) + " " + str(file_list[n]))
        filename, filetype = os.path.splitext(file_list[n])
        
        # Round the corners with default percent of radius
        curr_image = image_list[n]
        curr_code = qr_image_list[n]
        new_image = modify_one_image(curr_image, "insaniTea", curr_code) 
