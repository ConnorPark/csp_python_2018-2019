...
###Kinds of borders for people to use
#-------------------------------------------------------------------------------
    ...
    for row in range(width):
        for column in range(height):
            ...
            if column in range(height):
                image[row][column] = [0, 0, 0, 225]#Black
            #####if row >= 100: 
                #(r+c)/w says how many stripes above/below line y=x
                # The % 2 says whether it is an even or odd stripe
...
def checkeredpattern(width, height, original_image):
    '''
    Outputs an image with a mask thats a checkered pattern.
    Takes the dimensions of the image and takes the image itself.T
    Gets every other 200 x 200 box in the image and replaces it with the color 
    of the users choice.
    '''
    ...
    #for every box with dimensions of 200,200 , remove it and replace it with black.
    ...
    mask_image3 = PIL.Image.fromarray(image3)
    mask_image3 = mask_image3.resize((width, height))
    
    mask_in_use = mask_image3
    result = PIL.Image.new('RGBA', original_image.size, (0,0,0,0))
...
def border(width, height, original_image):
    ...
    for row in range(width):
        for column in range(height):
            image2[row][column] = [0, 0, 0, 0] #Black, all transparent
    
    for row in range(width):
        for column in range(height):
            if row in range(width // 6, 5 * width // 6) and column in range(height // 6, 5 * height // 6):
                image2[row][column] = [0, 0, 0, 255] #Black, Fully visible
...
def compare_and_contrast_mask(width, height, original_image, current_result):
    ...
    partial_curr_result.paste(current_result, (0,0))#Pastes image at 0,0
...
def combine(width, height, original_image, curr_result):
    ...
    new_image.paste(curr_result, (width, 0))
...
###End of the Masks
#-------------------------------------------------------------------------------
def interactivesettings(original_image, chosen_mask, code, percent_of_side=.3):
    """
    ...
    print ("Would you like a poster format?")
    poster = raw_input()
    print ()
    ...
    if width <= 500 or height <= 500:
        ...
        width2, height2 = logo.size
        #If the image is too small, then make the image smaller
...
#------------------------------------------------------------------------------- 
###Makes it easier for the client to pick a color. Client just needs to type 
###the name of the color that they want
    ...
    elif picking_color == 'cyan':
        colorchose = (0,225,255,255)
    else:
        colorchose = (0,0,0,0)   
    
#-------------------------------------------------------------------------------
###Logic for each item the client chooses. Goes back to the function of the 
###Clients options function
...
#        result = PIL.Image.alpha_composite(result, shading_for_image(width, height, original_image))
    ...
    #    result.paste(original_image, (0,0), mask=mask_in_use)
    if poster.lower() == "yes":
        result.paste(original_image, (0,0), mask=poster )
    ...
    #new code
    text_width = len(text) * (width // 20)
    
    add_text = PIL.ImageDraw.Draw(result)
#    add_text.text(((width // 2) - (text_width // 2), (height // 2)), text, font=font, fill=(255, 255, 255, 255))
    
     
    if picking_mask.lower() == "combine": #not working
        ...
        result = new_image
        #Tried to make a poster but it didnt work
    
    return result
#-------------------------------------------------------------------------------
###Gets all of the images and organizes them and puts unique QR codes on all 
###of the images.
...
#-------------------------------------------------------------------------------
###Saves images to a modified folder
def saveimages(directory=None):
    ...
    for n in range(len(image_list)):
        ...
        new_image = interactivesettings(curr_image, "insaniTea", curr_code) 
        
        # Save the altered image, suing PNG to retain transparency
        new_image_filename = os.path.join(new_directory, filename + '.png')
        new_image.save(new_image_filename)
        
def poster(original_image):
