...
def modify_one_image(original_image, chosen_mask, code, percent_of_side=.3):
    ...
    picking_mask = raw_input("Which mask would you like? Rounded mask, Border mask, insaniTea or combine (to combine the original and result)? ")
...
'''def combine_images(directory=None):
    """ Saves a modfied version of each image in directory.
    ...
    # Create a new directory 'modified'
    new_directory = os.path.join(directory, 'combo-images')
    try:
        os.mkdir(new_directory)
    except OSError:
        pass # if the directory already exists, proceed  
    ...
    # Load all the images
    image_list, file_list = get_images("combo", directory)
    qr_image_list, qr_file_list = get_qr_codes(directory)

    # Go through the images and save modified versions
    for n in range(len(image_list)):
        filename, filetype = os.path.splitext(file_list[n])
        if (n + 1) == len(image_list):
            print("Image "+str(n+1) + " " + str(file_list[n])+"Image "+str(1) + " " + str(file_list[0]))
            curr_image = image_list[n]
            curr_comp_image = image_list[n+1]
            curr_code = qr_image_list[n]
            new_image = combine(width, height, curr_image, curr_comp_image)
        # Parse the filename
        else:
            print("Image "+str(n+1) + " " + str(file_list[n])+"Image "+str(n+1) + " " + str(file_list[n]+1))
